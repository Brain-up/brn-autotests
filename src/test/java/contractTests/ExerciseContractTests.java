...
public class ExerciseContractTests extends InitTest {
    ...
    private final String EXERC_SINGLE_WORD_SERIES_SCHEMA = SCHEMA.value + "exercises_for_single_words.json";
    ...
    public void checkExerciseContract() {
        given().spec(rspec).when().queryParam(SERIES_ID.value, "1").get("").then()
                .assertThat().body(matchesJsonSchemaInClasspath(EXERC_SINGLE_WORD_SERIES_SCHEMA)).statusCode(200).log().all();
    ...
    public void checkExerciseContract_3() {
        given().spec(rspec).when().queryParam(SERIES_ID.value, "2").get("").then()
                .assertThat().body(matchesJsonSchemaInClasspath(EXERC_SEQ_WORD_SERIES_SCHEMA)).statusCode(200).log().all();
    ...
    public void checkExerciseContract_5() {
        given().spec(rspec).when().get("/1").then()
                .assertThat().body(matchesJsonSchemaInClasspath(EXERC_SCHEMA)).statusCode(200).log().all();
